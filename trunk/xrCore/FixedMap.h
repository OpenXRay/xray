...
private:
	...
	void		Realloc()
	{
		...
		TNode*	newNodes = (TNode*)TAlloc::alloc(sizeof(TNode)*newLimit);
		VERIFY(newNodes);
	...
	IC void		getLR(TNode* N,
		xr_vector<T, typename TAlloc::template helper<T>::result>&	D)
	{
		if (N->left)	getLR(N->left, D);
		D.push_back(N->val);
		if (N->right)	getLR(N->right, D);
	}
	IC void		getRL(TNode* N,
		xr_vector<T, typename TAlloc::template helper<T>::result>& D)
	{
	...
	IC void		getLR_P(TNode* N,
		xr_vector<TNode*, typename TAlloc::template helper<TNode*>::result>& D)
	{
...
public:
	...
	IC void		discard()	{ if (nodes) TAlloc::dealloc(nodes); nodes = 0; pool = 0; limit = 0; }
	...
	IC void		getLR(xr_vector<T, typename TAlloc::template helper<T>::result>&	D)
	{
		if (pool)	getLR(nodes, D);
	}
	IC void		getLR_P(xr_vector<TNode*, typename TAlloc::template helper<TNode*>::result>&	D)
	{
		if (pool)	getLR_P(nodes, D);
	}
	IC void		getRL(xr_vector<T, typename TAlloc::template helper<T>::result>&	D)
	{
		if (pool)	getRL(nodes, D);
	}
	IC void		getRL_P(xr_vector<TNode*, typename TAlloc::template helper<TNode*>::result>&	D)
	{
		if (pool)	getRL_P(nodes, D);
	}
	IC void		getANY(xr_vector<T, typename TAlloc::template helper<T>::result>&	D)
	{
		TNode*	_end = end();
		for (TNode* cur = begin(); cur != _end; cur++) D.push_back(cur->val);
	}
	IC void		getANY_P(xr_vector<TNode*, typename TAlloc::template helper<TNode*>::result>&	D)
	{
		D.resize(size());
		TNode** _it = &*D.begin();
		TNode*	_end = end();
	...
	IC void		getANY_P(xr_vector<void*, typename TAlloc::template helper<void*>::result>&	D)
	{
		D.resize(size());
		void** _it = &*D.begin();
		TNode*	_end = end();
