...
*/
	...
	if((mode==MPxFileTranslator::kExportAccessMode)||(mode==MPxFileTranslator::kSaveAccessMode)||(mode==MPxFileTranslator::kExportActiveAccessMode))
		status = (m_bReferenceFile)?exportObject(fname):exportMotion(fname);
...
MStatus CXRaySkinExport::exportObject(LPCSTR fn)
{
	MStatus status;
	...
	OBJECT->m_Flags.set(CEditableObject::eoDynamic,TRUE);
...
MStatus CXRaySkinExport::exportMotion(LPCSTR fn)
{
	R_ASSERT(!m_bReferenceFile);
	string256 nm;
	strcpy(nm,fn);
	...
	// cout << "Exporting animation frames from " << frameFirst << " to " << frameLast << "\n";
	{
		...
		for (int i=frameFirst; i<=frameLast; i++)
		{
			...
			for (SmdBoneIt boneIt = m_boneList.begin(); boneIt != m_boneList.end(); boneIt++) 
			{
				...
				float h,p,b, x,y,z;
				x=bone->trans.x; y=bone->trans.y; z=bone->trans.z;
				h=bone->orient.y;p=bone->orient.x;b=bone->orient.z;
	...
	if (!pSkinObject) {
		...
		for ( ; !iter.isDone(); iter.next() ) {
			...
			if (mapIt == m_jointMap.end()) {
				...
				// cout << "Bone found: " << joint.name().asChar();
				if (joint.isFromReferencedFile(&status)) {
					MString nspace = joint.parentNamespace (&status);
...
MStatus CXRaySkinExport::getBoneData (void)
{
	MStatus status;
	...
	if (status==MS::kSuccess){
		for (SmdBoneIt itBones = m_boneList.begin(); itBones != m_boneList.end(); itBones++){
			...
			if ((*itBones)->parentId==-1)
				mat = CalculateFullTransform(fnJoint);
