...
const char *const xraySkinDefaultOptions	=
	"SkinCluster=;"
	;

const char *const xraySkinMotionOptionScript	= "xraySkinMotionExportOptions";
const char *const xraySkinMotionDefaultOptions	=
	"SkinCluster=;"
;
...
//	MSceneMessage::addCallback(MSceneMessage::kMayaExiting,  uninitialize, 0);
	...
	status = plugin.registerFileTranslator("XRay_Object_Export",		"none", CXRayObjectExport::creator, (char *)xrayObjectOptionScript, (char *)xrayObjectDefaultOptions );
	if (status!=MS::kSuccess) return status;
	status = plugin.registerFileTranslator("XRay_Skin_Export",			"none", CXRaySkinExport::creator_skin, (char *)xraySkinOptionScript, (char *)xraySkinDefaultOptions);
	if (status!=MS::kSuccess) return status;
	status = plugin.registerFileTranslator("XRay_Skin_Motion_Export",	"none", CXRaySkinExport::creator_skin_motion, (char *)xraySkinMotionOptionScript, (char *)xraySkinMotionDefaultOptions);
	if (status!=MS::kSuccess) return status;
