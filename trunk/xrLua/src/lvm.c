/*
** $Id: lvm.c,v 2.54 2005/08/29 20:49:21 roberto Exp $
** Lua virtual machine
...
//.
LUA_API int LUA_CC luaV_strcmp(const TString *ls, const TString *rs) { return l_strcmp(ls,rs); }
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_ADD: {
        ...
        if (ttisnumber(rb) && ttisnumber(rc)) {
          ...
          Protect(luaV_arith(L, ra, rb, rc, TM_ADD));
        continue;
      }
      case OP_SUB: {
        TValue *rb = RKB(i);
        ...
        if (ttisnumber(rb) && ttisnumber(rc)) {
          ...
          Protect(luaV_arith(L, ra, rb, rc, TM_SUB));
        continue;
      }
      case OP_MUL: {
        TValue *rb = RKB(i);
        ...
        if (ttisnumber(rb) && ttisnumber(rc)) {
          ...
          Protect(luaV_arith(L, ra, rb, rc, TM_MUL));
        continue;
      }
      case OP_DIV: {
        TValue *rb = RKB(i);
      ...
      case OP_POW: {
        ...
        if (ttisnumber(rb) && ttisnumber(rc)) {
          ...
          Protect(luaV_arith(L, ra, rb, rc, TM_POW));
        continue;
      }
      case OP_UNM: {
        TValue *rb = RB(i);
