...
#ifndef _WIN32	// we need BUFSIZ to be defined in windows
#ifdef BUFSIZ
#undef BUFSIZ
#endif
#endif
...
/*
**  At the time of this writing, the only platform concerned
**  by this lines is SGI running IRIX 3.3.x
*/
...
#pragma no side effects (fexp);
#pragma no side effects (fsin);
#pragma no side effects (fcos);
#pragma no side effects (ftan);
#pragma no side effects (fasin);
#pragma no side effects (facos);
#pragma no side effects (fatan);
#pragma no side effects (ffloor);
#pragma no side effects (flog);
#pragma no side effects (flog10);
#pragma no side effects (fsqrt);
...
#define	FLmakeid(c1,c2,c3,c4)	((unsigned long)(c4)				| \
				((unsigned long)(c3) << 8)			| \
				((unsigned long)(c2) << 16)			| \
				((unsigned long)(c1) << 24))
...
#define	FLuswapword(wp)		((unsigned long)(((unsigned char *)wp)[3])		| \
				((unsigned long)(((unsigned char *)wp)[2]) << 8)	| \
				((unsigned long)(((unsigned char *)wp)[1]) << 16)	| \
				((unsigned long)(((unsigned char *)wp)[0]) << 24))
...
#define	FLuswapleword(wp)	((unsigned long)(((unsigned char *)wp)[0])		| \
				((unsigned long)(((unsigned char *)wp)[1]) << 8)	| \
				((unsigned long)(((unsigned char *)wp)[2]) << 16)	| \
				((unsigned long)(((unsigned char *)wp)[3]) << 24))
...
/*
**	this macro builds reversed ids on little endian machines;
...
#define	FLuswapleword(wp)	((unsigned long)(((unsigned char *)wp)[3])		| \
				((unsigned long)(((unsigned char *)wp)[2]) << 8)	| \
				((unsigned long)(((unsigned char *)wp)[1]) << 16)	| \
				((unsigned long)(((unsigned char *)wp)[0]) << 24))
...
#endif
#define	FL_Mem		0x00000080
#define	FL_Mapped	0x00000100
...
typedef INT_PTR		 TintPtrSz;
typedef	UINT_PTR	 TuIntPtrSz;
typedef INT          Tint32;
typedef UINT         TuInt32;
#elif (defined(SGI) || defined(LINUX))
#include <inttypes.h>
typedef intptr_t	 TintPtrSz;
typedef uintptr_t	 TuIntPtrSz;
typedef int          Tint32;
...
typedef	struct _FLfile
{
    ...
    void *		includes;
#ifdef _WIN64
#pragma message ("PK: Medium Risk Change --> " __FILE__)
#endif
    FLid		userdata;
    char *		unrb;
    int			unrs;
...
#define	FLbsword(w)		(FLbshalf((unsigned long)(w) >> 16) +		\
				 FLbshalf((unsigned long)(w) & 0xffff))
...
#define	FLrevword(w)		(FLrevhalf((unsigned long)(w) >> 16) +		\
				 (FLrevhalf((unsigned long)(w) & 0xffff) << 16)
...
#ifdef LINUX
extern char *GETENV(const char *name) __attribute__ ((alias ("getenv")));
extern int PUTENV(const char *name) __attribute__ ((alias ("putenv")));
#endif /* LINUX */
